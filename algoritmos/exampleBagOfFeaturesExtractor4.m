function [features, featureMetrics, varargout] = exampleBagOfFeaturesExtractor4(I)
...
X = 1:gridStep:width-9;
Y = 1:gridStep:height-9;


[x,y] = meshgrid(X, Y);
W = ones(1,length(x(:)))*gridStep; 
H = ones(1,length(y(:)))*gridStep;
gridLocations = [x(:) y(:) W' H'];

%%
% Concatenate multiple SURFPoint objects at different scales to achieve
% multiscale feature extraction.
multiscaleGridPoints = detectSURFFeatures(grayImage,'MetricThreshold', 100);
...
features = extractFeatures(grayImage, multiscaleGridPoints,'Upright',false);
